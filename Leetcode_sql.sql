...
select
country_name ,
case when average_weather <=15 then 'Cold'
    when average_weather>=25 then 'Hot'
    else 'Warm' end as weather_type 
from Countries c
inner join (select country_id, avg(Weather_state) average_weather 
           from weather 
           where date_format(day, '%Y-%m')= '2019-11'  #yyyy-mm expression
           group by country_id
          ) w
